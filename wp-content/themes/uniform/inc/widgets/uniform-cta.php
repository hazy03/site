...
class Uniform_Cta extends WP_Widget {
    ...
    public function widget( $args, $instance ) {
        ...
        $uniform_cta_title          = empty( $instance['title'] ) ? '' : $instance['title'];
        $uniform_cta_content        = empty( $instance['action_text'] ) ? '' : $instance['action_text'];
        $uniform_cta_button_link    = empty( $instance['action_btn_link'] ) ? '' : $instance['action_btn_link'];
        $uniform_cta_button_text    = empty( $instance['action_btn_text'] ) ? '' : $instance['action_btn_text'];
        $uniform_cta_aside_style    = empty( $instance['inline'] ) ? '' : 'aside-style';
        
        echo $before_widget;

        ?>
        <div class="section-actionbox <?php echo esc_attr( $uniform_cta_aside_style ); ?>">
            <div class="action-wrap">
                <?php
                    if ( $uniform_cta_title ) echo $before_title . esc_html( $uniform_cta_title ) . $after_title;
                    if ( !empty( $uniform_cta_content ) ) { 
                ?>
                    <div class="action-content"><?php echo esc_textarea( $uniform_cta_content ); ?></div>
                <?php } ?>
            </div>
            <?php if ( ! empty( $uniform_cta_button_link ) ) { ?>
...
<?php 
     ...
     *
     * @uses	uniform_widgets_show_widget_field()		defined in uniform-widget-fields.php
     */
    public function form( $instance ) {
        ...
        foreach ( $widget_fields as $widget_field ) {
            ...
            extract( $widget_field );
            $uniform_widgets_field_value = ! empty( $instance[$uniform_widgets_name] ) ? wp_kses_post( $instance[$uniform_widgets_name] ) : '';
